...
! hydrogen, but with targ.Mtar_struck=targ.M, targ.Mtar_rec=bound final state.
	...
	if (doing_hyd_elast) then	!p = q
     ...
     >	    write(6,*) 'cos(phi)=',vertex%up%x/sin(vertex%p%theta)
	  ...
	  call rotate3d(vertex%p%P*vertex%up%x,vertex%p%P*vertex%up%y,vertex%p%P*vertex%up%z,spec%p%theta,spec%p%phi,vertex%p%xptar,vertex%p%yptar,vertex%p%theta,vertex%p%phi)
...
! So looking along q, out of plane is down, perp. is left.
	...
	vertex%Pmx = -(vertex%p%P*vertex%up%x - vertex%q*vertex%uq%x) ! Negative sign for convention to match hcana
...
!phi is projection on x-y plane (so phi=0 is down, sos=pi/2, hms=3*pi/2.
	...
	subroutine rotate3d(up0x,upy0,upz0,theta0,phi0,dx0,dy0,theta,phi)
