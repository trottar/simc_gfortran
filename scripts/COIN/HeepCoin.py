...
# Description:
# ================================================================
# Time-stamp: "2022-09-26 16:36:40 trottar"
# ================================================================
#
...
G_data_eff = ROOT.TGraphErrors(len(InData_efficiency.split(' ')), np.array([float(x) for x in data_runNums.split(' ')]),np.array([float(x) for x in InData_efficiency.split(' ')]),np.array([0]*len(tot_effError)),np.array(tot_effError))
G_dummy_eff = ROOT.TGraphErrors(len(InDummy_efficiency.split(' ')), np.array([float(x) for x in dummy_runNums.split(' ')]),np.array([float(x) for x in InDummy_efficiency.split(' ')]),np.array([0]*len(tot_effError)),np.array(tot_effError))
...
for evt in TBRANCH_DATA:
  ...
  if(HMS_FixCut & HMS_Acceptance & SHMS_FixCut & SHMS_Acceptance):
      ...
      H_ct_ep_DATA.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DATA)
      
      H_ssxfp_DATA.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DATA)
      H_ssyfp_DATA.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DATA)
      H_ssxpfp_DATA.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DATA)
      H_ssypfp_DATA.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DATA)
      H_ssdelta_DATA.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DATA)
      H_ssxptar_DATA.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DATA)
      H_ssyptar_DATA.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DATA)
      
      H_hsxfp_DATA.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DATA)
      H_hsyfp_DATA.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DATA)
      H_hsxpfp_DATA.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DATA)
      H_hsypfp_DATA.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DATA)
      H_hsdelta_DATA.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DATA)
      H_hsxptar_DATA.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DATA)
      H_hsyptar_DATA.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DATA)

      H_ph_q_DATA.SetBinError(ibin,eff_errProp*evt.H_ph_q_DATA)
      H_th_q_DATA.SetBinError(ibin,eff_errProp*evt.H_th_q_DATA)
      H_ph_recoil_DATA.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DATA)
      H_th_recoil_DATA.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DATA)
      
      H_pmiss_DATA.SetBinError(ibin,eff_errProp*evt.H_pmiss_DATA)
      H_emiss_DATA.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA)
      #H_emiss_DATA.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA)
      H_pmx_DATA.SetBinError(ibin,eff_errProp*evt.H_pmx_DATA)
      H_pmy_DATA.SetBinError(ibin,eff_errProp*evt.H_pmy_DATA)
      H_pmz_DATA.SetBinError(ibin,eff_errProp*evt.H_pmz_DATA)
      H_Q2_DATA.SetBinError(ibin,eff_errProp*evt.H_Q2_DATA)
      H_W_DATA.SetBinError(ibin,eff_errProp*evt.H_W_DATA)
      H_epsilon_DATA.SetBinError(ibin,eff_errProp*evt.H_epsilon_DATA)
      H_MMp2_DATA.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA)
      #H_MMp2_DATA.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA)
      #H_MMp2_DATA.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA)

      H_cal_etottracknorm_DATA.SetBinError(ibin,eff_errProp*evt.H_cal_etottracknorm_DATA)
      H_cer_npeSum_DATA.SetBinError(ibin,eff_errProp*evt.H_cer_npeSum_DATA)

      P_cal_etottracknorm_DATA.SetBinError(ibin,eff_errProp*evt.P_cal_etottracknorm_DATA)
      P_hgcer_npeSum_DATA.SetBinError(ibin,eff_errProp*evt.P_hgcer_npeSum_DATA)
      P_aero_npeSum_DATA.SetBinError(ibin,eff_errProp*evt.P_aero_npeSum_DATA)
...
for evt in TBRANCH_DATA_nocut:
  ...
  H_ct_ep_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DATA_nocut)

  H_ssxfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DATA_nocut)
  H_ssyfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DATA_nocut)
  H_ssxpfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DATA_nocut)
  H_ssypfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DATA_nocut)
  H_ssdelta_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DATA_nocut)
  H_ssxptar_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DATA_nocut)
  H_ssyptar_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DATA_nocut)

  H_hsxfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DATA_nocut)
  H_hsyfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DATA_nocut)
  H_hsxpfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DATA_nocut)
  H_hsypfp_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DATA_nocut)
  H_hsdelta_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DATA_nocut)
  H_hsxptar_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DATA_nocut)
  H_hsyptar_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DATA_nocut)

  H_ph_q_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ph_q_DATA_nocut)
  H_th_q_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_th_q_DATA_nocut)
  H_ph_recoil_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DATA_nocut)
  H_th_recoil_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DATA_nocut)

  H_pmiss_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_pmiss_DATA_nocut)
  H_emiss_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA_nocut)
  #H_emiss_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA_nocut)
  H_pmx_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_pmx_DATA_nocut)
  H_pmy_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_pmy_DATA_nocut)
  H_pmz_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_pmz_DATA_nocut)
  H_Q2_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_Q2_DATA_nocut)
  H_W_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_W_DATA_nocut)
  H_epsilon_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_epsilon_DATA_nocut)
  H_MMp2_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nocut)
  #H_MMp2_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nocut)
  #H_MMp2_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nocut)

  H_cal_etottracknorm_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_cal_etottracknorm_DATA_nocut)
  H_cer_npeSum_DATA_nocut.SetBinError(ibin,eff_errProp*evt.H_cer_npeSum_DATA_nocut)

  P_cal_etottracknorm_DATA_nocut.SetBinError(ibin,eff_errProp*evt.P_cal_etottracknorm_DATA_nocut)
  P_hgcer_npeSum_DATA_nocut.SetBinError(ibin,eff_errProp*evt.P_hgcer_npeSum_DATA_nocut)
  P_aero_npeSum_DATA_nocut.SetBinError(ibin,eff_errProp*evt.P_aero_npeSum_DATA_nocut)
...
for evt in TBRANCH_DATA_nopid:
  ...
  if(HMS_FixCut & HMS_Acceptance & SHMS_FixCut & SHMS_Acceptance):
      ...
      H_ct_ep_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DATA_nopid)
      
      H_ssxfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DATA_nopid)
      H_ssyfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DATA_nopid)
      H_ssxpfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DATA_nopid)
      H_ssypfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DATA_nopid)
      H_ssdelta_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DATA_nopid)
      H_ssxptar_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DATA_nopid)
      H_ssyptar_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DATA_nopid)
      
      H_hsxfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DATA_nopid)
      H_hsyfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DATA_nopid)
      H_hsxpfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DATA_nopid)
      H_hsypfp_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DATA_nopid)
      H_hsdelta_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DATA_nopid)
      H_hsxptar_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DATA_nopid)
      H_hsyptar_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DATA_nopid)

      H_ph_q_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ph_q_DATA_nopid)
      H_th_q_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_th_q_DATA_nopid)
      H_ph_recoil_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DATA_nopid)
      H_th_recoil_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DATA_nopid)
      
      H_pmiss_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_pmiss_DATA_nopid)
      H_emiss_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA_nopid)
      #H_emiss_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_emiss_DATA_nopid)
      H_pmx_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_pmx_DATA_nopid)
      H_pmy_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_pmy_DATA_nopid)
      H_pmz_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_pmz_DATA_nopid)
      H_Q2_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_Q2_DATA_nopid)
      H_W_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_W_DATA_nopid)
      H_epsilon_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_epsilon_DATA_nopid)
      H_MMp2_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nopid)
      #H_MMp2_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nopid)
      #H_MMp2_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DATA_nopid)

      H_cal_etottracknorm_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_cal_etottracknorm_DATA_nopid)
      H_cer_npeSum_DATA_nopid.SetBinError(ibin,eff_errProp*evt.H_cer_npeSum_DATA_nopid)

      P_cal_etottracknorm_DATA_nopid.SetBinError(ibin,eff_errProp*evt.P_cal_etottracknorm_DATA_nopid)
      P_hgcer_npeSum_DATA_nopid.SetBinError(ibin,eff_errProp*evt.P_hgcer_npeSum_DATA_nopid)
      P_aero_npeSum_DATA_nopid.SetBinError(ibin,eff_errProp*evt.P_aero_npeSum_DATA_nopid)
...
for evt in TBRANCH_DUMMY:
  ...
  if(HMS_FixCut & HMS_Acceptance & SHMS_FixCut & SHMS_Acceptance):
      ...
      H_ct_ep_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DUMMY)
      
      H_ssxfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DUMMY)
      H_ssyfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DUMMY)
      H_ssxpfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DUMMY)
      H_ssypfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DUMMY)
      H_ssdelta_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DUMMY)
      H_ssxptar_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DUMMY)
      H_ssyptar_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DUMMY)
      
      H_hsxfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DUMMY)
      H_hsyfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DUMMY)
      H_hsxpfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DUMMY)
      H_hsypfp_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DUMMY)
      H_hsdelta_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DUMMY)
      H_hsxptar_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DUMMY)
      H_hsyptar_DUMMY.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DUMMY)

      H_ph_q_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ph_q_DUMMY)
      H_th_q_DUMMY.SetBinError(ibin,eff_errProp*evt.H_th_q_DUMMY)
      H_ph_recoil_DUMMY.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DUMMY)
      H_th_recoil_DUMMY.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DUMMY)
      
      H_pmiss_DUMMY.SetBinError(ibin,eff_errProp*evt.H_pmiss_DUMMY)
      H_emiss_DUMMY.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY)
      #H_emiss_DUMMY.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY)
      H_pmx_DUMMY.SetBinError(ibin,eff_errProp*evt.H_pmx_DUMMY)
      H_pmy_DUMMY.SetBinError(ibin,eff_errProp*evt.H_pmy_DUMMY)
      H_pmz_DUMMY.SetBinError(ibin,eff_errProp*evt.H_pmz_DUMMY)
      H_Q2_DUMMY.SetBinError(ibin,eff_errProp*evt.H_Q2_DUMMY)
      H_W_DUMMY.SetBinError(ibin,eff_errProp*evt.H_W_DUMMY)
      H_epsilon_DUMMY.SetBinError(ibin,eff_errProp*evt.H_epsilon_DUMMY)
      H_MMp2_DUMMY.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY)
      #H_MMp2_DUMMY.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY)
      #H_MMp2_DUMMY.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY)
...
for evt in TBRANCH_DUMMY_nocut:
  ...
  H_ct_ep_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DUMMY_nocut)

  H_ssxfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DUMMY_nocut)
  H_ssyfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DUMMY_nocut)
  H_ssxpfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DUMMY_nocut)
  H_ssypfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DUMMY_nocut)
  H_ssdelta_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DUMMY_nocut)
  H_ssxptar_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DUMMY_nocut)
  H_ssyptar_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DUMMY_nocut)

  H_hsxfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DUMMY_nocut)
  H_hsyfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DUMMY_nocut)
  H_hsxpfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DUMMY_nocut)
  H_hsypfp_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DUMMY_nocut)
  H_hsdelta_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DUMMY_nocut)
  H_hsxptar_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DUMMY_nocut)
  H_hsyptar_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DUMMY_nocut)

  H_ph_q_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ph_q_DUMMY_nocut)
  H_th_q_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_th_q_DUMMY_nocut)
  H_ph_recoil_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DUMMY_nocut)
  H_th_recoil_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DUMMY_nocut)

  H_pmiss_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_pmiss_DUMMY_nocut)
  H_emiss_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY_nocut)
  #H_emiss_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY_nocut)
  H_pmx_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_pmx_DUMMY_nocut)
  H_pmy_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_pmy_DUMMY_nocut)
  H_pmz_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_pmz_DUMMY_nocut)
  H_Q2_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_Q2_DUMMY_nocut)
  H_W_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_W_DUMMY_nocut)
  H_epsilon_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_epsilon_DUMMY_nocut)
  H_MMp2_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nocut)
  #H_MMp2_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nocut)
  #H_MMp2_DUMMY_nocut.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nocut)
...
for evt in TBRANCH_DUMMY_nopid:
  ...
  if(HMS_FixCut & HMS_Acceptance & SHMS_FixCut & SHMS_Acceptance):
      ...
      H_ct_ep_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ct_ep_DUMMY_nopid)
      
      H_ssxfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxfp_DUMMY_nopid)
      H_ssyfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssyfp_DUMMY_nopid)
      H_ssxpfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxpfp_DUMMY_nopid)
      H_ssypfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssypfp_DUMMY_nopid)
      H_ssdelta_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssdelta_DUMMY_nopid)
      H_ssxptar_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssxptar_DUMMY_nopid)
      H_ssyptar_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ssyptar_DUMMY_nopid)
      
      H_hsxfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxfp_DUMMY_nopid)
      H_hsyfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsyfp_DUMMY_nopid)
      H_hsxpfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxpfp_DUMMY_nopid)
      H_hsypfp_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsypfp_DUMMY_nopid)
      H_hsdelta_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsdelta_DUMMY_nopid)
      H_hsxptar_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsxptar_DUMMY_nopid)
      H_hsyptar_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_hsyptar_DUMMY_nopid)

      H_ph_q_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ph_q_DUMMY_nopid)
      H_th_q_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_th_q_DUMMY_nopid)
      H_ph_recoil_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_ph_recoil_DUMMY_nopid)
      H_th_recoil_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_th_recoil_DUMMY_nopid)
      
      H_pmiss_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_pmiss_DUMMY_nopid)
      H_emiss_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY_nopid)
      #H_emiss_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_emiss_DUMMY_nopid)
      H_pmx_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_pmx_DUMMY_nopid)
      H_pmy_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_pmy_DUMMY_nopid)
      H_pmz_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_pmz_DUMMY_nopid)
      H_Q2_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_Q2_DUMMY_nopid)
      H_W_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_W_DUMMY_nopid)
      H_epsilon_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_epsilon_DUMMY_nopid)
      H_MMp2_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nopid)
      #H_MMp2_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nopid)
      #H_MMp2_DUMMY_nopid.SetBinError(ibin,eff_errProp*evt.H_MMp2_DUMMY_nopid)
